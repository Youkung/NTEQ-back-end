...
function generateID(currentMaxID, prefix) {
    ...
    // Increment and pad with zeros to 9 digits
...
async function getNextID(tableName, idField, prefix) {
    try {
        ...
        const newID = `${prefix}${nextNum.toString().padStart(9, '0')}`;
...
app.get('/api/items/:equipmentId', async (req, res) => {
    ...
    try {
        let whereClause = 'i.Equipe_ID = ?';
        let params = [req.params.equipmentId];

        if (search) {
            whereClause += ' AND i.Serial_Number LIKE ?';
            params.push(`%${search}%`);
        }
        ...
        const [items] = await db.query(
            `SELECT 
             ...
             WHERE ${whereClause}
             ORDER BY i.Item_CreateDate DESC
             LIMIT ? OFFSET ?`,
            [...params, limit, offset]
        );

        res.json({
            items: items || [], // Ensure we always return an array
            pagination: {
                currentPage: page,
...
app.delete("/api/item/:Item_ID", async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            success: true,
            message: "ลบอุปกรณ์สำเร็จ",
            items: [] // Return empty array after deletion
        });
...
app.put("/api/item/:Item_ID", async (req, res) => {
    ...
    try {
        ...
        if (currentItem[0].Item_Status !== Item_Status) {
            ...
            const nextNum = (result[0].maxNum || 0) + 1;
            const newStatusID = `S${nextNum.toString().padStart(9, '0')}`;
...
app.get("/api/note/user/:userId", async function (req, res) {
    ...
    try {
        ...
        const [notes] = await db.query(`
            ...
            INNER JOIN user_nt u ON n.User_ID = u.User_ID 
            WHERE n.User_ID = ?
            ORDER BY n.Note_CreateDate DESC
            LIMIT ? OFFSET ?
        `, [userId, limit, offset]);
...
app.get("/api/user/profile", async function (req, res) {
    ...
    if (!authHeader) {
        return res.status(401).json({ error: "ไม่พบข้อมูลการยืนยันตัวตน" });
    }
...
app.get("/api/users", async function (req, res) {
    ...
    try {
        const [countResult] = await db.query('SELECT COUNT(*) as total FROM user_nt');

        // Include Password field in the SELECT statement
        const [users] = await db.query(`
            SELECT 
                user_nt.User_ID,
                user_nt.Username,
...
app.put("/api/user/:id", async (req, res) => {
    ...
    if (!firstName || !lastName || !username || !phone || !email || !roleId) {
        return res.status(400).json({ message: "กรุณากรอกข้อมูลให้ครบถ้วน" });
    }
    ...
    try {
        ...
        if (password) {
            sql = `
                ...
                WHERE User_ID = ?
            `;
            params = [username, password, `${firstName} ${lastName}`, phone, email, roleId, id];
        } else {
            sql = `
                UPDATE user_nt 
                SET 
                ...
                WHERE User_ID = ?
            `;
            params = [username, `${firstName} ${lastName}`, phone, email, roleId, id];
        }
        ...
        // Fetch updated user data with role name
...
app.post("/api/users", async function (req, res) {
    ...
    try {
        // Check if the username already exists (keep this check)
...
app.get("/api/note", async function (req, res) {
    const page = parseInt(req.query.page) || 1;
    ...
    try {
        ...
        const [notes] = await db.query(`
            ...
            LEFT JOIN user_nt ON note.User_ID = user_nt.User_ID
            LEFT JOIN role ON user_nt.Role_ID = role.Role_ID
            ORDER BY note.Note_CreateDate DESC
            LIMIT ? OFFSET ?
        `, [limit, offset]);
    ...
    try {
        ...
        if (Note_Images && Note_Images.length > 0) {
            for (const imagePath of Note_Images) {
                ...
                const newImageID = `I${nextNum.toString().padStart(9, '0')}`;
        ...
        res.status(500).json({
            success: false,
            message: "เกิดข้อผิดพลาดในการสร้างบันทึก",
            error: error.message
        });
    } finally {
        if (connection) {
...
app.get("/api/rooms", async (req, res) => {
    ...
    try {
      ...
      SELECT COUNT(*) as total 
      FROM (
        SELECT DISTINCT node.Node_ID, room.Room_ID
        FROM node 
...
app.get("/api/rooms/search", async (req, res) => {
    const { branch_number, branch_name, building, floor, room, page = 1 } = req.query;
    const limit = 10;
    const offset = (page - 1) * limit;
    ...
    try {
        ...
        if (branch_number) {
            conditions.push('node.Node_Location LIKE ?');
        ...
        if (building) {
            conditions.push('node.Node_Building LIKE ?');
    ...
    `;
        ...
        const [rooms] = await db.query(sql, [...values, limit, offset]);
...
app.post("/api/upload-chunk", async (req, res) => {
    try {
        ...
        if (result) {
            res.status(200).json({ message: "อัปโหลดรูปภาพสำเร็จ", imagePath: result });
        } else {
            res.status(200).json({ message: "อัปโหลดชิ้นส่วนสำเร็จ" });
        }
    } catch (error) {
...
app.post("/api/note", async function (req, res) {
    ...
    try {
        ...
        res.status(201).json({ message: "สร้างบันทึกสำเร็จ", Note_ID: newNoteID });
    } catch (error) {
        console.error("Error creating note:", error);
        res.status(500).json({ message: "เกิดข้อผิดพลาดในการสร้างบันทึก", error: error.message });
    }
});
...
app.get("/api/equipment/search", async function (req, res) {
    const { Serial_Number, Node_Name, Room_Name, Object_Name, Item_Status, page = 1, limit = 10 } = req.query;
    const offset = (page - 1) * limit;
    ...
    try {
        ...
        const [countResult] = await db.query(countSql, values);
        const [data] = await db.query(dataSql, paginationValues);
        ...
        console.error("Error in equipment search:", err);
        res.status(500).json({
            data: [],
            error: err.message
        });
...
app.post("/api/rooms/node", async (req, res) => {
    ...
    try {
        ...
        const nextNum = (result[0].maxNum || 0) + 1;
        const newNodeID = `N${nextNum.toString().padStart(9, '0')}`;
...
app.post("/api/rooms/room", async (req, res) => {
    ...
    try {
        ...
        const newRoomID = `R${nextRoomNum.toString().padStart(9, '0')}`;
        ...
        const newObjectID = `OBJ${nextObjectNum.toString().padStart(9, '0')}`;
...
app.post("/api/objects", async (req, res) => {
    ...
    if (!roomId || !objectName) {
        return res.status(400).json({ error: "ต้องระบุรหัสห้องและชื่อพื้นที่" });
    }
    ...
    try {
        ...
        const objectId = `OBJ${nextId.toString().padStart(9, '0')}`;
        ...
        res.status(201).json({ message: "สร้างพื้นที่สำเร็จ", objectId });
    } catch (error) {
        console.error("Error creating object:", error);
        res.status(500).json({ error: "เกิดข้อผิดพลาดในการสร้างพื้นที่" });
    }
});
...
app.delete("/api/objects/:objectId", async (req, res) => {
    ...
    try {
        ...
        if (items[0].count > 0) {
            return res.status(400).json({
                success: false,
                message: "ไม่สามารถลบพื้นที่ที่มีอุปกรณ์อยู่ได้"
            });
        }
            ...
            if (object.item_ids) {
                ...
                const [itemHistories] = await db.query(`
                    ...
                    LEFT JOIN item_history ih 
                        ON i.Item_ID = ih.Item_ID
                        AND ih.Item_history_CreateDate = (
                            SELECT MAX(Item_history_CreateDate) 
                            FROM item_history 
                            WHERE item_history.Item_ID = i.Item_ID
                        )
                    WHERE i.Item_ID IN (?)
                `, [itemIds]);
...
app.post("/api/objects/:objectId/items", async (req, res) => {
    ...
    try {
        ...
        if (existingItem.length > 0) {
            ...
            return res.status(400).json({
                success: false,
                message: "มีหมายเลขซีเรียลนี้ในระบบแล้ว"
            });
        }
        ...
        const nextId = (maxId[0].maxId || 0) + 1;
        const itemId = `I${nextId.toString().padStart(9, '0')}`;
...
// Add this new endpoint for getting complete object data
app.get("/api/object/:objectId", async (req, res) => {
    try {
        const [object] = await db.query(
            `SELECT o.*, n.Node_Name, r.Room_Name 
       FROM object o
       JOIN node n ON o.Node_ID = n.Node_ID
       JOIN room r ON o.Room_ID = r.Room_ID
       WHERE o.Object_ID = ?`,
...
app.put("/api/note/:Note_ID", async (req, res) => {
    const { Note_ID } = req.params;
    const { User_ID, Note_Head, Note, Note_CreateDate, Note_Images } = req.body;
    const formattedDate = Note_CreateDate ? new Date(Note_CreateDate).toISOString().slice(0, 19).replace('T', ' ') : null;
    let connection;
    ...
    try {
        ...
        // Update note
        await connection.query(
            "UPDATE note SET User_ID = ?, Note_Head = ?, Note = ?, Note_CreateDate = ?, Note_LastModifiedDate = NOW() WHERE Note_ID = ?",
            [User_ID, Note_Head, Note, formattedDate, Note_ID]
        );

        // Delete existing images that are not in the new set
        const existingImagePaths = Note_Images.filter(path => !path.startsWith('data:'));
        await connection.query(
            "DELETE FROM note_images WHERE Note_ID = ? AND Image_Path NOT IN (?)",
            [Note_ID, existingImagePaths.length ? existingImagePaths : ['dummy']]
        );
        ...
        if (Note_Images && Note_Images.length > 0) {
            for (const imagePath of Note_Images) {
                ...
                const [existingImage] = await connection.query(
                    "SELECT Image_ID FROM note_images WHERE Note_ID = ? AND Image_Path = ?",
                    [Note_ID, imagePath]
                );
        ...
        await connection.commit();
        res.status(200).json({ message: "อัปเดตบันทึกสำเร็จ" });
    } catch (error) {
        if (connection) {
            await connection.rollback();
        }
        console.error("Error updating note:", error);
        res.status(500).json({ message: "เกิดข้อผิดพลาดในการอัปเดตบันทึก", error: error.message });
    } finally {
        if (connection) {
            connection.release();
        }
